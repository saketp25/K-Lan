...
function signUp() {
	var uname = document.getElementById('newUname');
	var nPass = document.getElementById('newPw');
	var repPass = document.getElementById('repPw');
	if(uname.value === '' || nPass.value === '')
	{
		alert('Invalid credentials');
		return false;
	}
	else if(nPass.value !== repPass.value)
	{
	...
	else
	{
	    ...
	    xmlhttp.open("POST", "Signup?uname=" + uname.value + "&pw=" + nPass.value, true);
	    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
            if (xmlhttp.responseText.indexOf('Incorrect') !== -1) {
                document.getElementById("loading-icon").innerHTML = "<h4 style='color:red'>" + xmlhttp.responseText + "<h4>";
            } else {
                document.getElementById("signup-data").innerHTML = '<h1>' + xmlhttp.responseText + '</h1>';
            }
        }
